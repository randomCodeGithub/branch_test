...
$section->addListItem( 'Käesolev mobiiltelefonivõrgu (roaming) kaudu tagatava telekommunikatsiooniteenuse osutamise leping (edaspidi Leping) sõlmiti CSC Telecom Estonia OÜ (edaspidi CSC TELECOM) ja ' . $info['company_name'] . ' (edaspidi KLIENT) vahel.', 1, '', 'multilevel', 'p_justify' );
...
$cell = $table->addCell( \PhpOffice\PhpWord\Shared\Converter::cmToTwip( 7.41 ) );
$cell->addText( 'Klient: ' . $info['company_name'], 'f_bold', 'p_left' );
$cell->addText( 'Aadress: ' . $info['address'], '', 'p_left' );
$cell->addText( 'Registrikood: ' . $info['reg_code'], '', 'p_left' );
$cell->addText( 'VAT number: ' . $info['vat_number'], '', 'p_left' );
...
$textrun = $section->createTextRun( 'p_justify' );
$textrun->addText( $info['company_name'], 'f_bold', 'p_justify' );
$textrun->addText( ' (KLIENT), keda esindab juhatuse liige ', '', 'p_justify' );
...
$section->addListItem( 'Kõikidele tariifidele kohaldub käibemaks ja neid näidatakse ilma käibemaksuta.', 0, '', 'multilevel-3', '' );
